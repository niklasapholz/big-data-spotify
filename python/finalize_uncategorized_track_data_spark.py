...
if __name__ == '__main__':
  """
  Create new dataframe with all necessary information to reduce size
  """
  args = get_args()
  ...
  for category in playlists_with_id.keys():
    ...
    finalized_df = finalized_df.withColumn('release_date', finalized_df['album']['release_date'] )
    finalized_df = finalized_df.withColumn('artists', concat_ws(', ',finalized_df['artists']['name']))
    finalized_df = finalized_df.withColumn('song_title', finalized_df['name'])
    finalized_df = finalized_df.select('id', 'song_title', 'album_type', 'release_date', 'artists')
